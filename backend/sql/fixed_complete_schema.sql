-- Complete Schema Setup for ytFetch (FIXED)
...
-- =====================================================
-- PART 2: GUEST ACCESS SYSTEM (FIXED)
-- =====================================================

-- Guest usage tracking (FIXED - indexes moved outside)
...
-- Create indexes for guest_usage table (FIXED - now separate)
CREATE INDEX IF NOT EXISTS idx_guest_session_method ON guest_usage(session_id, method);
CREATE INDEX IF NOT EXISTS idx_guest_created_at ON guest_usage(created_at);
...
-- Show all tables in public schema with their purpose
SELECT 
    tablename,
    CASE 
        ...
        WHEN tablename = 'user_profiles' THEN '👤 User Profiles'
        ELSE '📄 Other'
    END as system
FROM pg_tables 
